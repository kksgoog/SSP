...
from semi.core.hooks import Hook, get_priority, CheckpointHook, TimerHook, LoggingHook, DistSamplerSeedHook, ParamUpdateHook, EvaluationHook, EMAHook, WANDBHook, AimHook
from semi.core.utils import get_dataset, get_data_loader, get_optimizer, get_cosine_schedule_with_warmup, Bn_Controller
from semi.core.criterions import CELoss, ConsistencyLoss
...
class AlgorithmBase:
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.set_hooks()
        self.result_path = 'result1_unet_mix_wwwwww'
        self.train_acc = []
        self.train_loss = []
        self.train_top1 = []
    ...
    def plot_loss_and_acc(self, step):
        path = os.path.join(os.path.join(self.result_path, str(step)), 'train_loss&acc')
    ...
    def plot_top1(self, step):
        path = os.path.join(os.path.join(self.result_path, str(step)), 'train_acc')
        folder = os.path.exists(path)
        if not folder:
            os.makedirs(path)

        # np.savetxt(os.path.join(path, 'train_loss.txt'), self.train_loss)
        np.savetxt(os.path.join(path, 'train_top1.txt'), self.train_top1)

        # plt.plot(self.train_loss, label='train_loss')
        plt.plot(self.train_top1, label='train_top1')
        plt.legend()
    ...
    def evaluate(self, eval_dest='eval', out_key='logits', return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                out = logits.to(device=self.gpu)
                train_acc += get_acc(out, y)
                
                loss = F.cross_entropy(logits, y.long(), reduction='mean', ignore_index=-1)
        ...
        self.train_loss.append(float(total_loss/total_num))
        train_acc = np.array(train_acc/len(eval_loader))
        self.train_acc.append(float(train_acc))
        # if self.epoch % 10 == 0:
        #     self.plot_loss_and_acc(self.epoch)
        #     self.plot_top1(self.epoch)
        ...
        print('train')
        eval_dict = {eval_dest+'/loss': total_loss / total_num, eval_dest+'/top-1-acc': train_acc,
