...
from semi.core import AlgorithmBase
from semi.core.utils import ALGORITHMS
from semi.algorithms.utils import SSL_Argument, str2bool, mixup_one_target
...
class MixMatch(AlgorithmBase):
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        with self.amp_cm():
            ...
            # with torch.no_grad():
            # Pseudo Label
            input_labels = torch.cat([F.one_hot(y_lb.to(torch.int64), self.num_classes), sharpen_prob_x_ulb, sharpen_prob_x_ulb], dim=0)
            # Mix up
        ...
        return out_dict, log_dict, sup_loss.item(), unsup_loss.item(), total_loss.item(), mixed_y[num_lb:], logits_u
    ...
    def get_argument():
        return [
            ...
            SSL_Argument('--unsup_warm_up', float, 1 / 2, 'ramp up ratio for unsupervised loss'),
            SSL_Argument('--mixup_alpha', float, 1, 'parameter for Beta distribution of Mix Up'),
